...
const request = async (path, options = {}) => {
  const response = await fetch(`${baseUrl}${path}`, {
    headers: {
      ...
      ...(process.env.HIVE_SPIKE_TOKEN
        ? { authorization: `Bearer ${process.env.HIVE_SPIKE_TOKEN}` }
        : {}),
    },
    ...options,
  });
...
const writeRead = await request("/spike/write-read", {
  method: "POST",
  body: JSON.stringify(probe),
});
if (writeRead?.value !== nonce)
  throw new Error("Write/read round trip returned an unexpected value.");
const score = await request("/runs", {
  method: "POST",
  body: JSON.stringify({
    stage: "S-01",
