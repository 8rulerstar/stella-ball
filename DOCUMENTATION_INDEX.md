...
- 캠페인은 별자리 월드 구조다. `북두칠성` 7 스테이지와 `카시오페이아` 5 스테이지가 있고, 각 스테이지 이름은 실제 별 이름이다. **새 기믹을 소개하는 스테이지에는 그 기믹만 두고, 그 외에도 최대 두 종류까지만 둔다.** 무한 훈련장은 모든 기믹을 한꺼번에 켜 두는 유일한 곳이다.
- 로스터는 8명이며 시작 보유는 가온·비연·리아 3명이다. 기본 전투는 유성 5개, 일반 보스 체력 260, 단일 체력 전투다.
