...
| 달무리 · 기본 공명   | `assets/characters/sera-monk-idle.png`                                                                                                | 프로젝트 오너 제작 (2026-08-13)                      | 유성 접촉 시 함께 가속하며 별도의 고유 공격 없음                                                                                                                                                                                                                                           |
...
| 하늘 반응 오버레이   | `assets/original/sky/constellation-reaction-overlay.png`                                                                              | OpenAI 이미지 생성 · 프로젝트 오너 승인 (2026-08-23) | 768×512 투명 픽셀 오버레이. 5점 이상 별자리 현현과 세션 첫 보스 처치 때 양옆 여백에만 짧게 표시하며 중앙 전투 컬럼은 비운다. 런타임 연결은 `prototypes/sky-ambience.js`                                                                                                                    |

별자리 실루엣의 출처는 프로젝트 오너 발주 절차 생성이고, 라이선스는 **프로젝트 오너 원본 에셋(제3자 재배포 조건 미정)**으로 기록한다. 경로는 `assets/library/constellations/`, 런타임 연결점은 `prototypes/js/game-figure-recognition.js`의 `FIGURE_SHAPES[].art`와 `prototypes/js/game-figure.js`의 `drawFigure`다.
...
- 이 두 값은 `game-figure-recognition.js`의 `FIGURE_ART_SIZE`(384) · `FIGURE_ART_UNIT`(138)과 **반드시 일치**해야 한다
...
> **표의 「윤슬 — 런타임 회전 칼날 링」이 실제로 뜻하던 것**: `abilityFx` 표에
> 윤슬만 항목이 없어서, `emitAbilityFx`의 `if (asset)` 가드에 걸려 회전 칼날은
> 이펙트 버스트를 **한 번도** 만들지 않았다. 2026-08-24에 고쳤다 — 전용 시트가
> 있으면 벡터 자산 없이도 버스트가 서고, 칼날이 켜지는 `armBladeWheel`이
> 윤슬의 「시전」 자리가 된다.

### 별지기 전용 이펙트 시트 24장 (2026-08-24 반입)

위 표는 능력 «종류»별 한 장이라 여덟 명이 같은 그림을 썼다. 이 배치는 로스터
여덟 × 세 박자다 — **시전 · 정산 명중 · 각성 인장**.

| 규격        | 값                                                                                                                                           |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 시트        | 768×192, 가로 4프레임(192×192), 셀 8px, 알파 0/255                                                                                           |
| 경로        | `assets/library/anim/fx/<hero>-{swordwave,arrow,split,dash,bladewheel,orbit,shockwave,copy}.png` · `<hero>-impact.png` · `<hero>-awaken.png` |
| 색          | 각 별지기 `heroes[].col` 5단 + 외곽선. 장당 고유색 5~6개                                                                                     |
| 예약색      | 미사용 — 살구 램프·안내별 금·CTA 금·경고 적색이 24장 어디에도 없다                                                                           |
| 재생        | 시전 `drawAbilityFx`(per-hero 가 per-kind 를 이긴다) · 각성 120ms/프레임 · 명중 90ms/프레임                                                  |
| 성능        | rAF p95 변화 **0ms** (`scripts/probe-hero-fx.mjs`, 시트 OFF/ON 을 같은 판에서)                                                               |
| 알려진 한계 | 달무리(`#bca7ff`)와 그믐(`#9f83ff`)이 hue 0.8° 차이라 여섯 장이 구분되지 않는다                                                              |

규격이 기존 `abilityFxSheets`와 같아서 `naturalWidth === naturalHeight * 4`
준비 검사와 그리기 코드를 그대로 쓴다 — 새 재생 경로가 없다.
