...
3. `game-data.js` — canvas/DOM bindings, balance constants, heroes, weapons (`WEAPONS`: common + starkeeper-exclusive, with the drop rate and cost), stages, asset paths, upgrades, shared mutable state, and small shared helpers.
...
8. `game-meta-state.js` — safe settings/progress persistence, reward and economy rules, the weapon inventory and equip map with its pull/equip/`weaponStatsFor` helpers, audio, achievements, and shared meta-state helpers.
   8b. `game-bgm.js` — real-track background music (title / hub / battle) played through `HTMLAudioElement`, mapped per scene and crossfaded, ducking the synthesised observatory score while a track plays. Presentation only; it loads right after `game-meta-state.js` so the audio engine and settings it reads already exist.
9. `game-meta.js` — settings, achievements, profile, shop, the starkeeper and weapon summon plus the weapon armory (all three under the summon screen's segmented tabs), stage-select, attendance, and library screens. It reacts to battle and roster lifecycle hooks but does not own their state machines.
...
13. `game-figure.js` — constellation ability dispatch, settlement/final-shot deferral, trace correction, reveal presentation, and the frozen `figure` API, including copied template points used by the guided lesson. It loads after recognition and before feedback. Sagitta fires along its `axis`; the other outcomes are kept in `FIGURE_ABILITIES`. All seven silhouette textures live in `../assets/library/constellations/`, and their coordinate contract is in `../ASSET_PLAN.md`.
    13b. `game-figure-cinematics.js` — per-constellation cast cinematics, loaded immediately after `game-figure.js` so its `afterDraw` hook paints over the corrected figure. Presentation only: every number still comes from `FIGURE_ABILITIES`. It is deliberately absent from the headless harness's `runtimeFiles` because it uses `Math.random` in twenty-odd places and that harness's seed reproducibility assumes randomness is drawn in exactly one spot; `CINE.orionKnock` (the one entry that moves the meteor) ships disabled for the same reason.
...
15. `game-onboarding.js` — story intro, first-session storage, the guided 1-1 lesson (awaken-first, all three party slots open from the start), observatory presentation, and onboarding extensions. It publishes a frozen `onboarding` API for session status and tutorial entry.
...
17. `game-awaken-fx.js` — the awakening presentation from `AWAKEN_FX_REQUEST_2026_08_16.md`, added as one file with no edits to the existing ones. It hangs off `afterShotStart`, `afterFeedbackUpdate`, `afterArenaDraw` and `afterDraw`. It writes `impactStop` and `screenShake`, which are the runtime's time and camera controls rather than gameplay state, so it stays in the `js/` chain instead of the presentation tier.
    17b. `game-hit-fx.js` — the settlement-impact vocabulary from `HIT_FX_REQUEST_2026_08_24.md`. It replaces `drawFinisherImpacts` by reassignment, the same way `game-awaken-fx.js` replaces `drawFinisherFocus`, so `game-feedback.js` is untouched and removing the `<script>` tag restores the original. Like that file it lives inside an IIFE: `scripts/smoke-runtime.mjs` forbids global function replacement and detects it by a line-start regex, so an indented reassignment is the house-sanctioned form — and it keeps the eleven vocabulary helpers out of the global namespace. `HIT_FX_VOCAB` at the top picks one of four vocabularies (`burst` / `pierce` / `crater` / `flash`); the default is `burst`. Presentation only: it reads `impact.fromX/fromY` (the striking starkeeper, for direction) and `impact.amount` (damage, for scale) and writes nothing.
...
Eight presentation-only scripts load after that chain and stay outside `js/` because they never touch gameplay state:
...
23. `stella-ball-pixel-ui.js` — exposes `window.StellaPixelUI`; renders pixel button silhouettes and decor sprites to canvas data URLs.
    23b. `stella-ball-weapon-icons.js` — the twelve `wpn<Id>` 16×16 dot maps for the weapon icons (design-session intake). It merges them into `StellaPixelUI.MAPS`, so it must load after `stella-ball-pixel-ui.js`; the glyph→dot swap itself is a `weaponIcons()` pass in `stella-ball-upgrade.js`. Data only, no global functions.
...
26. `sky-ambience.js` — the margin sky's three parallax layers, autonomous idle cycles, and the event background reactions from `SKY_AMBIENCE_REQUEST_2026_08_14.md`. A `#dawn-sky` child like the intro, loaded between `stella-ball-dawn.js` and `outer-observer.js` so the intro draws above it. It hooks `afterFigureResolve` for 5+ point constellations and `afterBattleWin` for the session's first clear; `window.SkyAmbience` keeps the manual QA surface (`figure` / `world` / `boss` / `blaze` / `setProgress` / `layout`).
...
With Node.js 20 or newer, run `npm run check` from the repository root. The verifier reads the 17 gameplay-owning runtime files as one logical bundle — the other six in `prototypes/js/` stay outside it (`game-figure-cinematics.js`, `game-speech.js`, `game-awaken-fx.js`, `game-hit-fx.js`, `game-bgm.js`, `game-perfwatch.js`: cast, speech, fx and audio presentation plus a hand-run instrument; no gameplay marker lives there) — checks required gameplay markers and asset references, runs portability checks, and writes `artifacts/verification.json` (`runtimeFiles`: the HTML + 5 stylesheets + those 17). The smoke test concatenates and parses **every** script the HTML loads except the `../hive/` ones, and reports the totals itself (`runtimeScripts` / `stylesheets`) — read those two numbers from the gate rather than from this sentence, because they drift whenever a file is added. It validates exact script order, zero duplicate global functions, module immutability, hook ordering, contract rejection, and all three deferred-figure outcomes. The final `test:onboarding` pass opens a fresh headless Chromium profile and drives the real card journey (`ONBOARDING_CARD_COUNT`, currently 13) through selection, awakening, the guided constellation, final battle, and reward screen. Its scope and fixture are recorded in [ONBOARDING-E2E](./ONBOARDING-E2E.md). `npm run serve` starts a local server at `http://127.0.0.1:4173/`.
