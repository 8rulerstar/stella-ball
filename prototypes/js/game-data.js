...
const WORLDS = [
  {
    ...
    name: "양자리",
    bayer: "ARIES",
    lore: "세 별빛을 잇는 첫 관측 항로. 별빛 조준과 첫 별자리를 배운다.",
    shape: [
      [13, 62],
      [48, 27],
      [87, 55],
  ...
  },
  {
    ...
    name: "오리온자리",
    bayer: "ORION",
    lore: "여섯 별의 사냥꾼. 조향 한 번과 공명 접점을 함께 회수한다.",
    shape: [
      [12, 62],
      [28, 27],
      [46, 48],
...
const CAMPAIGN_WORLD_PLANS = [
  ...
  },
  {
    ...
    stages: [
      ...
      ["사드르", "γ Cyg", "교차점", 305, "두 공명 접점을 한 샷에 이어 보세요."],
      [
        "중앙 깃",
        "δ Cyg",
        "흐르는 선",
...
const STAGE_GIMMICK_TRIAL = {
  ...
  /* 2-3·2-4 는 월드 2의 «뒷판»이라 둘을 겹친다(설계 규칙: 겹치기는 뒤에서만).
     실측 근거: 가속판만 있는 판은 바닥 정책이 100%로 깬다 — 세기를 더해
     주기만 하니 난이도가 아니라 선물이다. 시위의 긴장은 「좁게 겨누고 힘은
     판에서 빌린다」인데, 빌리는 데 값이 없으면 저울질이 아니다.
     빠른 길에 흐린 여울을 붙여 값을 만든다. */
  "2-3": {
    boostPads: [{ x: 360, y: 372, w: 132, h: 30, boost: 320 }],
    dragPads: [{ x: 360, y: 268, w: 172, h: 34, drop: 0.5 }],
  },
  "2-4": {
    boostPads: [
    ...
    dragPads: [
      { x: 226, y: 262, w: 140, h: 34, drop: 0.6 },
      { x: 494, y: 262, w: 140, h: 34, drop: 0.6 },
    ],
  },
...
let build,
  ...
  aimHover = -1,
  /* 키보드 조준 커서. aimHover와 «같은 인덱스 공간»(aimNodes()의 자리)을
     쓰지만 주인이 다르다 — aimHover는 pointermove가 매 프레임 덮어쓰고,
     캔버스에 pointerleave 핸들러가 없어 마우스를 노드 위에 세워 두면 그
     값이 영영 고정된다. 둘을 한 변수로 합치면 키보드로 옮긴 커서가 입력도
     없이 마우스 자리로 끌려간다. 읽을 때만 aimFocus()로 합친다. */
  aimCursor = -1,
  /* 노드 조준이 안 서는 판(노드 셋 미만)에서 키보드가 겨누는 «끄는 점».
     마우스는 그런 판에서 드래그로 쏘는데, 키보드에는 그 대응물이 없어
     캠페인 1-1(별지기 둘)에서 한 발도 못 쐈다. 각도와 당긴 거리를 들고
     있다가 발사할 때 드래그와 «같은 계산»을 지난다 — 안내선도 드래그의
     것을 그대로 쓴다. 마우스가 실제로 끌고 있으면(drag) 그쪽이 이긴다. */
  aimKeyPull = null,
  /* 조준을 무게중심 «반대편»으로 뒤집었는가. 무게중심은 언제나 노드들의
     볼록 껍질 안이라, 노드를 아무리 늘려도 겨눌 수 있는 각도가 껍질에
     갇힌다 — 실측으로 별빛을 상한(9개)까지 채워도 86도였다. 반대편을
     열면 껍질 «밖»이 열려 최악의 판에서도 219도가 된다. */
  aimFlip = false,
