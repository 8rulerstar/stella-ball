...
(() => {
  ...
  function pixelGem(x, cx, cy, r, ramp) {
    ...
    const bx = Math.round(cx / CELL), by = Math.round(cy / CELL);
  ...
  function wall(x, w, frameClock) {
    ...
    const hot = w.on > 0, heat = hot ? w.on / 0.22 : 0;
    const lateral = w.w >= w.h;
    const plates = 3, gapPx = 4;
    ...
    const x0 = snap(w.x - w.w / 2), y0 = snap(w.y - w.h / 2);
    hardShadow(x, x0 + 2, y0 + (lateral ? w.h - 2 : 2), w.w - 4, 6);
    for (let i = 0; i < plates; i++) {
      const bright = hot || i === lit;
      const spr = bake("wall:" + [wc, hc, hot ? 1 : 0, bright ? 1 : 0, lateral ? 1 : 0].join(":"), wc, hc, (P, cols, rows) => {
        const pal = hot
          ? { o: "#3a5560", hi: "#ffffff", b1: "#f6fefc", b2: "#dff2f2", b3: "#bcd9de", b4: "#95bcc5" }
          : { o: "#0e2028", hi: "#f2fbf9", b1: "#cfe6e8", b2: "#9dc2cf", b3: "#6f98a4", b4: "#446e7a" };
        const cut = 2;
        const inside = (a, b) => a >= 0 && b >= 0 && a < cols && b < rows && Math.min(a, cols - 1 - a) + Math.min(b, rows - 1 - b) >= cut;
        for (let b = 0; b < rows; b++) for (let a = 0; a < cols; a++) {
          if (!inside(a, b)) continue;
          if (!inside(a - 1, b) || !inside(a + 1, b) || !inside(a, b - 1) || !inside(a, b + 1)) { P(a, b, pal.o); continue; }
          let col;
        ...
        for (let dy = -3; dy <= 3; dy++) for (let dx = -3; dx <= 3; dx++)
          if (Math.abs(dx) + Math.abs(dy) <= 4 && inside(cx + dx, cy + dy)) P(cx + dx, cy + dy, hot ? "#7d5a33" : "#123640");
        chevStamp(P, cx, cy, lateral ? 1 : 2, bright ? (hot ? "#fffbe8" : "#d9fbff") : "#5d858f");
        for (const [rx, ry] of [[2, 2], [cols - 3, 2], [2, rows - 3], [cols - 3, rows - 3]]) P(rx, ry, pal.b4);
      });
    ...
    if (hot) {
      ...
      x.fillStyle = "#fffbe8";
      if (lateral) for (let a = x0; a < x0 + w.w; a += CELL * 3) x.fillRect(a, y0 - reach, CELL, CELL);
      else for (let b = y0; b < y0 + w.h; b += CELL * 3) x.fillRect(x0 + w.w + reach, b, CELL, CELL);
      x.restore();
    } else {
      x.save();
      x.globalAlpha = 0.22;
      x.fillStyle = "#7cc6bb";
      if (lateral) for (let a = x0 + 4; a < x0 + w.w - 4; a += CELL * 4) x.fillRect(a, y0 + w.h + 2, CELL * 2, CELL);
      else for (let b = y0 + 4; b < y0 + w.h - 4; b += CELL * 4) x.fillRect(x0 + w.w + 2, b, CELL, CELL * 2);
      x.restore();
    }
  }
  ...
  function boostPad(x, pad, frameClock) {
    const clock = (frameClock || 0) / 1000;
    const hot = pad.on > 0;
    const wc = Math.floor(pad.w / CELL), hc = Math.floor(pad.h / CELL);
    const laneRows = Math.max(3, hc - 4);
    const step = Math.floor(clock * 8) % laneRows;
    const chevOn = Math.floor(clock * 6) % 3;
    const spr = bake("boost:" + [wc, hc, hot ? 1 : 0, chevOn, step].join(":"), wc, hc, (P, cols, rows) => {
      const railO = "#0f2418", rail = "#1f4030", railHi = "#4f8a52", rivet = "#8fc47a";
      const laneD = hot ? "#3f8a4b" : "#17382a", lane = hot ? "#63b061" : "#28543c", laneHi = hot ? "#d7ffb4" : "#7fae6b";
      for (let a = 0; a < cols; a++) {
        P(a, 0, railO); P(a, rows - 1, railO);
        if (a > 0 && a < cols - 1) { P(a, 1, a % 6 === 3 ? rivet : rail); P(a, rows - 2, a % 6 === 3 ? rivet : rail); }
      }
      for (let b = 0; b < rows; b++) { P(0, b, railO); P(cols - 1, b, railO); if (b > 1 && b < rows - 2) { P(1, b, railHi); P(cols - 2, b, rail); } }
      for (let b = 2; b < rows - 2; b++) for (let a = 2; a < cols - 2; a++) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (let k = 0; k < 3; k++) {
          ...
          chevStamp(P, cx, Math.floor(rows / 2) + 2 - k * 3, 0, on ? (hot ? "#ffffff" : "#e9ffd2") : "#3f6e46");
        }
      }
    });
    const x0 = snap(pad.x - pad.w / 2), y0 = snap(pad.y - pad.h / 2);
  ...
  function dragPad(x, pad, frameClock) {
    ...
    const hot = pad.on > 0;
    const wc = Math.floor(pad.w / CELL), hc = Math.floor(pad.h / CELL);
    const spr = bake("drag:" + [wc, hc, hot ? 1 : 0].join(":"), wc, hc, (P, cols, rows) => {
      const o = "#0d191b", rimL = hot ? "#cfdad7" : "#5f7a77", rimD = "#31504e";
      for (let a = 0; a < cols; a++) { P(a, 0, o); P(a, rows - 1, o); if (a > 0 && a < cols - 1) { P(a, 1, a % 4 < 2 ? rimD : null); P(a, rows - 2, a % 4 < 2 ? rimL : null); } }
      for (let b = 1; b < rows - 1; b++) { P(0, b, o); P(cols - 1, b, o); if (b % 4 < 2) { P(1, b, rimD); P(cols - 2, b, rimD); } }
      for (let b = 2; b < rows - 2; b++) for (let a = 2; a < cols - 2; a++) {
      ...
      for (let i = 0; i < n; i++) {
        const cx = Math.floor(((i + 0.5) / n) * cols), cy = Math.floor(rows / 2);
        for (let dy = -3; dy <= 4; dy++) for (let dx = -3; dx <= 3; dx++)
          if (Math.abs(dx) + Math.abs(dy) <= 4) P(cx + dx, cy + dy, "#1c1930");
  ...
  function bumper(x, b, frameClock) {
    const clock = (frameClock || 0) / 1000;
    const hot = b.on > 0;
    const R = Math.round((b.r + 7) / CELL), rMain = Math.round(b.r / CELL);
    const bx = Math.round(b.x / CELL), by = Math.round(b.y / CELL);
    for (let yy = -R; yy <= R; yy++) for (let xx = -R; xx <= R; xx++) {
      const d = Math.hypot(xx, yy), a = Math.atan2(yy, xx);
      let col = null;
      if (d <= R && d > R - 1.5) col = "#081418";
      else if (d <= R - 1.5 && d > R - 2.5) col = "#12262c";
      else if (d <= rMain && d > rMain - 2.5) {
        const spec = a > -2.7 && a < -1.1;
        col = hot ? (spec ? "#ffffff" : "#e4f5d5") : spec ? "#8fe0d5" : "#3fa39c";
      } else if (d <= rMain - 2.5 && d > rMain - 3.5) col = hot ? "#9fdcc8" : "#26605e";
      else if (d <= rMain - 3.5) col = (xx + yy) % 2 ? "#11302f" : "#0e2827";
      if (col) { x.fillStyle = col; x.fillRect((bx + xx) * CELL, (by + yy) * CELL, CELL, CELL); }
    }
    x.fillStyle = hot ? "#ffe9a8" : "#2e6e6a";
    for (const [sx, sy] of [[-1, -(rMain + 3)], [-1, rMain + 1], [-(rMain + 3), -1], [rMain + 1, -1]])
      x.fillRect((bx + sx) * CELL, (by + sy) * CELL, CELL * 2, CELL * 2);
    pixelGem(x, b.x, b.y, Math.max(9, b.r - 14), hot ? ["#c9a94f", "#ffe9a8", "#fff8df"] : ["#8a6f2e", "#e8cf77", "#fff3c9"]);
    const sa = clock * 1.6;
    x.fillStyle = "#f2fffb";
    x.fillRect((bx + Math.round(Math.cos(sa) * (rMain - 1))) * CELL, (by + Math.round(Math.sin(sa) * (rMain - 1))) * CELL, CELL, CELL);
  }
  ...
  function orbital(x, o) {
    ...
    const cx = o.x - Math.cos(o.a) * radius, cy = o.y - Math.sin(o.a) * radius;
    const halfSpan = Math.min(0.6, (o.r + 8) / radius + 0.08);
    const rOut = radius + 12, rIn = radius - 12;
    const a0 = o.a - halfSpan, a1 = o.a + halfSpan;
    const pal = hot
      ? { tooth: "#fff6e0", hi: "#ffe3c0", body: "#8a5f38", body2: "#6e4a2c", inner: "#2b1c10", cap: "#ffffff", stud: "#fff1d6" }
      : { tooth: "#b7e6de", hi: "#7cc6bb", body: "#2a4d57", body2: "#1c3944", inner: "#0e2028", cap: "#d8f2ea", stud: "#9adfc9" };
    const seen = new Set();
    const da = CELL / rOut;
    for (let a = a0; a <= a1; a += da) {
      ...
      for (let rr = rOut; rr >= rIn; rr -= CELL) {
        ...
        let col;
        if (t < 0.14) { if (!tooth && edgeK > da * 3) continue; col = pal.tooth; }
        else if (t < 0.32) col = pal.hi;
        else if (t < 0.66) col = pal.body;
        else if (t < 0.88) col = pal.body2;
        else col = pal.inner;
        const px = snap(cx + Math.cos(a) * rr), py = snap(cy + Math.sin(a) * rr);
        const key = px + ":" + py;
        if (seen.has(key)) continue;
        seen.add(key);
        if (col === pal.body && ((px + py) / CELL) % 2 && t > 0.5) col = pal.body2;
    ...
    for (const sa of [-halfSpan * 0.55, halfSpan * 0.55]) {
      ...
      x.fillRect(snap(cx + Math.cos(o.a + sa) * (radius + 3)) - CELL, snap(cy + Math.sin(o.a + sa) * (radius + 3)) - CELL, CELL * 2, CELL * 2);
    }
    pixelGem(x, o.x, o.y, 8, hot ? ["#8a5f38", "#ffd9a4", "#fff6e0"] : ["#123036", "#4db8b3", "#d8f2ea"]);
    // HP 핍 — 화면 정렬, 궤도 안쪽
    const tx = o.x - Math.cos(o.a) * 24, ty = o.y - Math.sin(o.a) * 24;
    const pips = 6, alive = Math.ceil((o.hp / o.maxHp) * pips);
    const px0 = snap(tx) - pips * 3, py0 = snap(ty);
    x.fillStyle = "#081418";
  ...
  function shieldRing(x, boss, shield) {
    ...
    const slots = Math.max(shield.max || shield.hits, shield.hits), gap = 0.16, span = (Math.PI * 2) / slots - gap,
      hot = shield.flash > 0, lift = hot ? snap(6 * (shield.flash / 0.45)) : 0;
    for (let i = 0; i < slots; i++) {
      const intact = i < shield.hits, justBroke = !intact && i === shield.hits && hot;
      if (!intact && !justBroke) continue;
      const a0 = -Math.PI / 2 + i * ((Math.PI * 2) / slots) + gap / 2,
        r = 78 + (justBroke ? snap(14 * (1 - shield.flash / 0.45)) + lift : lift);
      x.save();
      x.globalAlpha = justBroke ? shield.flash / 0.45 : 1;
      pixelArc(x, boss.x, boss.y, r, a0, a0 + span, hot ? SHIELD_L_HOT : SHIELD_L);
      for (const ea of [a0 + 0.02, a0 + span - 0.02]) {
        x.fillStyle = hot ? "#ffffff" : "#d8f2ea";
        x.fillRect(snap(boss.x + Math.cos(ea) * (r - CELL)), snap(boss.y + Math.sin(ea) * (r - CELL)), CELL, CELL * 2);
      }
      const midA = a0 + span / 2;
      x.fillStyle = hot ? "#ffffff" : "#eafaf4";
      x.fillRect(snap(boss.x + Math.cos(midA) * (r - CELL)) - CELL, snap(boss.y + Math.sin(midA) * (r - CELL)) - CELL, CELL * 2, CELL * 2);
      x.restore();
    }
  }
  // 걷힌 순간(0.4초). age는 소유자가 계산해 넘긴다 — 상태 소거도 소유자 몫.
  function shieldShatter(x, boss, shield, age) {
    const slots = shield.max || shield.shattered.count, gap = 0.16, span = (Math.PI * 2) / slots - gap;
    x.save();
    x.globalAlpha = 1 - age;
    for (let i = 0; i < shield.shattered.count; i++) {
      const a0 = -Math.PI / 2 + i * ((Math.PI * 2) / slots) + gap / 2;
      pixelArc(x, boss.x, boss.y, 78 + snap(age * 92), a0, a0 + span * (1 - age * 0.5), SHIELD_L_HOT);
    }
    x.restore();
  }
  ...
  function marker(x, s, frameClock) {
    ...
    const spr = bake("stone", 18, 36, (P) => {
      const o = "#0a161c", rim = "#9fe0d4", rimD = "#3f6a70", b1 = "#26505a", b2 = "#1a3a44", b3 = "#112a34", cap = "#33606a";
      for (let b = 1; b < 36; b++) {
        const inset = b < 3 ? 3 : b < 6 ? 2 : b < 32 ? 1 : 0;
        const x0 = inset, x1 = 17 - inset;
        for (let a = x0; a <= x1; a++) {
          let col;
      ...
      P(8, 0, rim); P(9, 0, rim);
      for (let i = 0; i < 3; i++) for (let a = 6; a < 12; a++) P(a, 25 + i * 3, (a + i) % 2 ? "#345257" : "#274850");
      P(8, 21, "#e8cf77"); P(9, 21, "#fff3c9"); P(8, 22, "#8a6f2e"); P(9, 22, "#e8cf77");
    });
    ...
    if (!s.lit) {
      x.fillStyle = "#57a79b";
      for (let i = 0; i <= 4; i++)
        for (const [dx, dy] of [[i, -4 + i], [-i, -4 + i], [i, 4 - i], [-i, 4 - i]])
          x.fillRect(Math.round(s.x / CELL + dx) * CELL - CELL, Math.round(sy / CELL + dy) * CELL, CELL, CELL);
    } else {
  ...
  window.StellaDotGimmicks = Object.freeze({
    CELL, wall, boostPad, dragPad, bumper, orbital, shieldRing, shieldShatter, marker, pixelArc, pixelGem,
  });
})();
