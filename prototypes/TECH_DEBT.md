...
3. **Before a release:** run `npm run check`, formatting, and whitespace checks. `check` already includes the browser onboarding path. Record performance numbers only when the runner and scenario are identical.
