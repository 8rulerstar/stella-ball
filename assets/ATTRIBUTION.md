...
- `assets/audio/sfx50/*.wav` — synthesised from scratch by two scripts in this repository and containing no recorded or sampled material: `scripts/generate_sfx_pack50.py` for the original fifty, and `scripts/generate_sfx_parry.py` (2026-08-13) for the eleven parry, starlight, constellation and summon cues. Both are deterministic and use only the Python standard library, so every file here can be rebuilt from source and none of it carries third-party terms.
- **The music has no files at all.** The observatory score is generated in Web Audio at runtime by `startObservatoryScore` in `prototypes/js/game-meta.js`, from the same two drone oscillators the build already had. Nothing is downloaded and nothing is licensed. Audio was deliberately not imported from the owner's other projects: the tracks in `~/Projects/WCF` and `~/Projects/TaleSurviver` are byte-identical to each other, are named as numbered album tracks, and carry no licence file in either project, so their terms are unknown and they must not be published from here.
