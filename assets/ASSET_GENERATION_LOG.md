...
- Codex는 현재 7인 전투 토큰과 같은 작은 16비트 RPG 화풍을 목표로 신규 유닛 3명을 생성했다. 결과는 균일한 마젠타 키 배경에서 투명 PNG로 변환하고, 알파 여백을 정리해 모두 `256×256` 규격으로 맞췄다.
...
- 사람은 기존 7인의 작은 전투 토큰과 동일한 가독성·크기·화풍을 먼저 확보하고, 능력은 나중에 전장 기믹과 함께 결정하기로 했다.
