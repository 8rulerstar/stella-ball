...
현재 프로토타입에 구현됨: 단계당 4발 제한, 보스 방벽 → 코어 노출 2단계, 전투 시작/코어 노출 시 강화 카드 선택, 연계·표식·반사·강화가 합산되는 피해 계산, 방벽/약점 피해 피드백 UI. 보스 몸체 어디에 맞혀도 기본 피해가 들어가며, 빛나는 약점은 1.8배 피해와 표식·치명 보정이 적용되는 고보상 목표다. 보스 명중에는 짧은 히트스톱·화면 흔들림·플래시·가능한 기기에서 햅틱을 주고, HUD는 실시간 운동량을 표시한다. 모든 룬 충돌은 공의 속도를 크게 보존·가속하고 광륜 피드백을 남긴다. 하루는 다음 파티 룬으로 공을 자동 전달하고, 세라는 정지한 공을 줄로 묶어 회전시킨 뒤 플레이어가 클릭한 타이밍에 충전된 원심력으로 발사한다. 닉스는 같은 공의 세 번째 충돌에 약점 유도탄을 만들고, 리오는 충돌 후 화면 클릭 한 번으로 공을 진행 방향 기준 오른쪽으로 90° 돌리며 운동량을 더한다. 이 능력들은 파티 순서와 궤적을 직접 바꾼다. 또한 균열 회랑·침식의 계단·원심 정원의 고정 룬석 좌표와 보스 위치가 서로 다르며, 파티를 고른 뒤 실제 픽셀 스프라이트를 드래그해 해당 스테이지의 룬석에 직접 배치한다.
