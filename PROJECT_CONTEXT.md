...
현재 프로토타입에 구현됨: 단계당 3발 제한, 보스 방벽 → 코어 노출 2단계, 전투 시작/코어 노출 시 강화 카드 선택, 연계·표식·반사·강화가 합산되는 피해 계산, 방벽/약점 피해 피드백 UI. 하루는 다음 파티 룬으로 공을 자동 전달하고, 세라는 정지한 공을 줄로 묶어 원심 재발사한다. 이 두 능력은 파티 순서와 궤적을 직접 바꾼다.
