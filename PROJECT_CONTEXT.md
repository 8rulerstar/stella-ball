...
현재 프로토타입에 구현됨: 단계당 3발 제한, 보스 방벽 → 코어 노출 2단계, 전투 시작/코어 노출 시 강화 카드 선택, 연계·표식·반사·강화가 합산되는 피해 계산, 방벽/약점 피해 피드백 UI. 하루는 다음 파티 룬으로 공을 자동 전달하고, 세라는 정지한 공을 줄로 묶어 원심 재발사한다. 닉스는 같은 공의 세 번째 충돌에 약점 유도탄을 만들고, 리오는 충돌 후 화면 클릭 한 번으로 공을 진행 방향 기준 오른쪽으로 90° 돌린다. 이 능력들은 파티 순서와 궤적을 직접 바꾼다. 또한 균열 회랑·침식의 계단·원심 정원의 고정 룬석 좌표와 보스 위치가 서로 다르며, 파티를 고른 뒤 실제 픽셀 스프라이트를 드래그해 해당 스테이지의 룬석에 직접 배치한다.
