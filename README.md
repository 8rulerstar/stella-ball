...
탑다운 당구 전장에서 유성과 3명의 별지기를 함께 굴리고, 충돌 순간의 `Space` 패링으로 각성·고유 능력·별빛을 만들어 공허 거상을 공략하는 브라우저 액션 전략 프로토타입입니다.
...
2. 시작 별지기 샛별·미리내·윤슬로 파티를 편성합니다. 일반 스테이지 클리어 보상 골드로 `별빛 소환`을 하면 나머지 별지기를 한 명씩 확정 해금할 수 있습니다.
3. 유성을 아래로 당겼다가 놓아 반대 방향으로 발사합니다. 첫 유성은 하단 발사석에서, 다음 유성은 이전 유성이 멈춘 자리에서 잇습니다.
4. 유성이 별지기와 닿는 순간 `Space`로 패링해 공명 각성·고유 능력·별빛 노드를 만듭니다. 한 샷의 노드 3~7개는 샷이 끝날 때 별자리로 발동합니다. 이동 중 좌·우클릭은 발사당 합산 한 번만 유성 궤적을 좌·우로 꺾습니다.
...
현재는 코어 플레이 프로토타입입니다. 캠페인은 양자리(3)부터 북두칠성(7)까지 7개 별자리 월드, 총 34스테이지이며 각 월드의 왼쪽 첫 노드부터 순차적으로 열린다. `1-1`은 첫 실행 온보딩이고, 양자리의 `1-2`·`1-3`은 첫 성공 패링에 안내별 둘을 보태 한 번의 3점 별자리를 보장한다. 그 외 전투는 보스 체력·시작 배치·패링 경로로 난도를 만든다. 일반 스테이지 클리어는 고정 100골드를 주며, `별빛 소환`은 100골드로 미보유 별지기 한 명을 확정 해금합니다. 유료 재화·확률형 중복 뽑기·실시간 멀티플레이와 Hive 실통신은 구현하지 않습니다. `무한 훈련장`은 기록 제출용이 아닌 물리·능력 QA용 전장입니다. 허브 지도 아래 버튼으로 들어가고, 기믹 없는 빈 판에 거상이 정중앙에 서며 파티가 4명입니다.
