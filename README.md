...
3. **조준** — 유성이 멈추면 판에 선 별지기와 남은 별빛 노드 중 **셋 이상**을 좌클릭으로 고르고 `Space`로 쏩니다. 방향은 고른 노드들의 무게중심이고 세기는 벌어진 정도라, **넓게 벌려 고를수록 세게 나갑니다.** 빈 곳을 누르면 반대편으로 쏘고, 우클릭·`Backspace`는 전부 무르기입니다. 고를 노드가 셋이 안 되는 판에서만 아래로 당겼다 놓는 드래그 발사로 돌아갑니다.
...
첫 실행에서는 루나의 관측 수업(카드 12장)이 먼저 열립니다. 모든 카드는 버튼을 눌러야 넘어가고 각 규칙은 처음 발동하기 전에 먼저 설명하며, 마지막 단계는 설명이 아니라 거상을 직접 처치하는 실전입니다.
