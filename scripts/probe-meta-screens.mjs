...
const AUDIT = `(() => {
  ...
  for (const el of [document.querySelector("#overlay"), ...document.querySelectorAll("#overlay *")]) {
    ...
    /* 편성 트레이는 «설계된» 가로 스크롤러다(2026-08-23 확인). 여덟 칸이
       406px 를 요구하는데 좁은 창에서는 트레이 폭이 255px 라 어떤 축소로도
       안 들어가고(코드는 이미 일곱부터 초상을 48→32 로 내린다), 화살표가
       붙은 스크롤바가 도달 경로로 그려진다. 도서관의 「뒤로」처럼 «다른
       길이 없는» 경우와 다르므로 여기서 세지 않는다 — 건강한 빌드에서
       늘 빨간불이면 이 프로브를 아무도 안 본다. */
    if (el.id === "squadTray") continue;
