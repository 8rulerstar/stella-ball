...
const tracked = execFileSync("git", ["ls-files", "-z"], { cwd: root }).toString("utf8")
  .split("\0")
  .filter(Boolean);
...
const assetReference = /(?:\.\.\/assets\/|assets\/)[A-Za-z0-9_./-]+\.(?:png|jpe?g|webp|gif|svg|woff2|mp3|ogg)/g;
...
for (const file of runtimeFiles) {
  ...
  for (const match of contents.matchAll(assetReference)) {
    ...
    const path = relative(root, resolve(dirname(resolve(root, file)), reference)).split(sep).join("/");
...
for (const file of tracked) {
  ...
  for (const part of file.split("/")) {
    if (/^(?:con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\..*)?$/i.test(part) || /[. ]$/.test(part)) {
...
console.log(JSON.stringify({ result: "passed", runtimeFiles: runtimeFiles.length, trackedFiles: tracked.length }, null, 2));
